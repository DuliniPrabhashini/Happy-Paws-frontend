...
import { petApi, petDetailsApi, diseaseApi } from "@/lib/api";
...
const Home: React.FC = () => {
  ...
  const [reminders, setReminders] = useState<PetDetail[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const remindersRes = await petDetailsApi.getPetDetailsReminder();
                  ...
                  <div className="space-y-4">
                    {reminders.length === 0 ? (
                      ...
                      reminders.map((reminder, index) => {
                        const Icon =
                          reminder.type === "VACCINE" ? Syringe : Calendar;
                        return (
                          <div
                            key={index}
                ...
                >
                  ...
                  <div className="mt-6 p-4 bg-hp-mint rounded-xl">
                    <p className="text-sm text-foreground">
                      <strong>💡 Tip:</strong> Regular health checkups can
                      prevent many common pet diseases. Schedule your next
                      appointment today!
                    </p>
                  </div>
                </div>
